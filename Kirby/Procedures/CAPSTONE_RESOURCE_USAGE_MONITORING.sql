...
FROM V$SESS_TIME_MODEL A
  INNER JOIN V$PX_SESSION B
    ON A.SID = B.SID
  INNER JOIN FINDING_SIDS_PGA_TOTALS C
    ON B.SID = C.SID
),
...
SELECT
  ...
  (SELECT VALUE FROM V$PARAMETER WHERE NAME = 'inmemory_size') AS INMEMORY_SIZE,
  (SELECT VALUE FROM V$PARAMETER WHERE NAME = 'parallel_degree_limit') AS PARALLEL_DEGREE_LIMIT,
  (SELECT VALUE FROM V$PARAMETER WHERE NAME = 'parallel_servers_target') AS PARALLEL_SERVERS_TARGET,
...
EXECUTE CAPSTONE_DEMO.RESOURCE_MONITOR.RESOURCE_USAGE_MONITORING_V2(1200, 5);
