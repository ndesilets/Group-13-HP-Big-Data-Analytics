...
SQL and most programming languages can be developed and excuted directly from a command line interface.
However since the early 1990s interactive developer enviroments or IDEs have been used to organize information and provide useful functionality to developers.
IDEs provide many useful features such data visualization or manipulation, code completion, usage finding, and much more. 
It is still possible to develope SQL on the command line but to do so would be a waste because it is ignoring the many tools at ones disposale. 
...
Code completion means the IDE is aware of tabel structures and data keys so the IDE can make reccomendations as the developer types.
...
It has a command line interface, and a windows graphical user interface, and a SQL plus web based user interface. 
SQL plus enables users to execute SQL, PL/SQL, and operating system commands to perfrom the following operations.
Format, perform calculations on, store, and print from query results.
Examine table and object functions.
...
SQL developer is a free stand alone graphical tool that can simplify database development. 
To get SQL developer one must download and install the application from the internet.
SQL developer is an IDE published by oracle and contains a vast array of features.
Some examples are worksheets for running queries, a DBA console, reports interface, and much more.
SQL developer offers all the features needed to complete end to end development of SQL applications.
...
Also because TOAD is paid for, the service tries to be much more proactive in helping the developer.
